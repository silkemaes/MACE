...
Once the Dataset class is set up properly (see `src/mace/CSE_0D/dataset.py <https://github.com/silkemaes/MACE/blob/main/src/mace/CSE_0D/dataset.py>`_), a MACE model can be trained. This can be done using the script 'run.py', which takes an input file with the needed (hyper)parameter setup. An example of such an input file can be found in input/.
