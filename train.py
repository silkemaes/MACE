...
name = str(arg) + '_' + str(now.strftime("%Y%m%d")+'_'+now.strftime("%H%M%S"))
...
print(f'\nMACE will be trained using {torch.get_num_threads()} threads')

## Load training & test data sets 
traindata, testdata, data_loader, test_loader = ds.get_data(data_type=input.data_type,dt_fract=input.dt_fract,
...
## Save results
print('\n\n>>> Training done! Saving results to', path)
# Losses
...
#     print(i+1,end='\r')
    ...
    err_test, err_evol, step_time, evol_time, _, _, _ = test.test_model(input.data_type,model,testpath, meta, printing = False, inpackage=True, datapath='train' )
...
print('\n>>> TRAINING SESSION DONE, GREAT SUCCESS!~')
