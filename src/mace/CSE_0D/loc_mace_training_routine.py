...
n_dim       = 468
nb_hidden   = 1 #int(inputfile['nb_hidden'])
ae_type     = 'simple' # str(inputfile['ae_type'])
nb_test     = 3000
...
tic = time()
tr.train(model, lr, data_loader, test_loader, path=path, end_epochs = ini_epochs, DEVICE= DEVICE, trainloss=trainloss, testloss=testloss, start_time=start)
...
## Continue training
tic = time()
tr.train(model, lr, data_loader, test_loader,  path=path, end_epochs = nb_epochs, DEVICE= DEVICE, trainloss=trainloss, testloss=testloss, start_epochs=ini_epochs, start_time=start)
toc = time()
train_time2 = toc-tic
...
## Continue training
tic = time()
tr.train(model, lr, data_loader, test_loader, path=path, end_epochs = tot_epochs, DEVICE= DEVICE, trainloss=trainloss, testloss=testloss, start_epochs=nb_epochs, start_time=start)
toc = time()
train_time3 = toc-tic
...
np.save(path+ '/testloss_step_logerr_' + str(len(trainset.testpath))+'.npy', np.array(sum_log_err_step))
